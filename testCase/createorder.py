...
class JiaDianOrder ( Init ) :
    ...
    def test1_tologin ( self ) :
        ...
        param = {"mobile" : self.mobile1 , "code" : self.code , "qd_no" : self.qd_no}
    ...
    def test2_addaddress( self ):
        '''用户创建服务地址'''
        url = self.default_url1+"v2/user/add-address?"+"qd_no="+self.qd_no+"&district_id=1997&access_token=" + self.getToken()
        form_data = {'username' : '研发'+random.choice("深圳欢迎您南山宝安福田罗湖龙岗株洲长沙"), 'sex': '' , 'phone' : self.mobile1, 'province_id' : 1895, 'city_id' : 1987,
                 ...
                 'street_code' : 440308001 , 'qd_no' : self.qd_no}
        data = form_data
        # print(url)
    ...
    def test3_createorder(self):
        ...
        url = self.default_url1 + "v2/order/add-order?qd_no="+self.qd_no+"&district_id=1990&access_token=" + self.getToken()
    ...
    def test4_topayorder ( self ) :
        ...
        param = {"pay_type" : "wallet" , "trade_type" : "order" , "qd_no" : self.qd_no , 'district_id' : 1990 ,
    ...
    def test5_booktime( self ) :
        '''用户预约上门服务时间'''
        sleep ( 2 )
        url = self.default_url1 + '/v2/order/modify-date?access_token=' + self.getToken() + '&qd_no=' + self.qd_no + '&district_id=1990'
        param = {"order_id" : self.getOrderid(), "date" : "2020-07-10" , "time" : "09:00-12:00" , 'type' : "" ,
                 'confirm_type' : "", 'qd_no' : self.qd_no}
